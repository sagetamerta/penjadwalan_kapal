...
public class Penjadwalan extends JFrame {
	...
	private String[] header_5 = { "Iterasi", "Individu", "Constraint 1", "Constraint 2", "Constraint 3", "Constraint 4",
			"Constraint 5", "Kromosom", "Nilai Fitness" };
	...
	public void hitungFitness() {
		try {
			...
			/*
			 * for(int i=0; i<popsize; i++) { double[] arr = new double[maxData]; for (int
			 * j=0; j < maxData; j++) { arr[j] = fitness[i][j]; } temp =
			 * Arrays.toString(arr); tbmodel_evaselek.addRow(new Object[] {i+1, temp}); }
			 */
	...
	public void seleksiElitism() {
		...
		/*
		 * for(int i=0; i<popsize; i++) { Object[] obj = new Object[2]; obj[0] = i+1;
		 * obj[1] = fitness[count][0]; tbmodel_evaselek.addRow(obj); }
		 */
